...
# Make the data ready for neural network training
def load_data_wrapper():
    tr_d,va_d,te_d = load_data()
    training_inputs = [np.reshape(x,(1,784)) for x in tr_d[0]] # reshape each image into vector of shape (1, 784) for input of neural network
    training_labels = [vectorize(x) for x in tr_d[1]]          # take each label and encode in one-hot encoding format
...
class Network(object):
    ...
    def update_mini_batch(self, mini_batch, eta, n, lamda):
        ...
        self.biases[-1] -= eta*grad_b * (1.0/float(len(mini_batch)))
        ...
        for l in range(2,self.num_layers):
            ...
            grad_b = np.sum(delta,axis=0,keepdims=True)
            self.biases[-l] -= eta*grad_b * (1.0/float(len(mini_batch)))
